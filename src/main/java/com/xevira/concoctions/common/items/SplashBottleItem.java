...
public class SplashBottleItem extends Item {

	public SplashBottleItem(Properties properties) {
		super(properties);
	}
		...
		if (raytraceresult.getType() == RayTraceResult.Type.MISS) {
				...
				if (raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
					...
					if (worldIn.getFluidState(blockpos).isTagged(FluidTags.WATER)) {
						...
						return ActionResult.func_233538_a_(this.turnBottleIntoItem(itemstack, playerIn, PotionUtils.addPotionToItemStack(new ItemStack(Items.SPLASH_POTION), Potions.WATER)), worldIn.isRemote());
					}
				}
				return ActionResult.resultPass(itemstack);
		}
